...
class QueryBuilder
{
    ...
    /**
     * Return PublicSummon search parameters based on a user query and params.
     *
    ...
    protected function queryToString(Query $query)
    {
        ...
        // Check if the query already contains fielded search syntax
        // If it does, we should not wrap it again to avoid double fielding
        if ($this->containsFieldedSearchSyntax($lookfor)) {
            return $lookfor;
        }
    ...
    /**
     * Check if the query string already contains fielded search syntax
     *
     * @param string $queryString The query string to check
     *
     * @return bool
     */
    protected function containsFieldedSearchSyntax(string $queryString): bool
    {
        // Pattern to match fielded search syntax:
        // - field:(value) - with parentheses
        // - field:value - without parentheses
        // - field:"value" - with quotes
        // This matches common field names followed by : and then either:
        // - (value) - parentheses enclosed
        // - "value" - quoted
        // - value - plain value (but not at the start of string to avoid false positives)
