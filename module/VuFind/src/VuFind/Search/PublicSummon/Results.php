 ...
 *
 * @category VuFind
 * @package  Search_PublicSummon
 * @author   Demian Katz <demian.katz@villanova.edu>
 * @license  http://opensource.org/licenses/gpl-2.0.php GNU General Public License
 * @link     https://vufind.org Main Page
 */

namespace VuFind\Search\PublicSummon;
...
/**
 * PublicSummon Search Parameters
 *
 * @category VuFind
 * @package  Search_PublicSummon
 * @author   Demian Katz <demian.katz@villanova.edu>
...
class Results extends \VuFind\Search\Base\Results
{
     ...
     * @var string
     */
    protected $backendId = 'PublicSummon';
    ...
    protected function performSearch()
    {
        ...
        if (!empty($dateFacets)) {
            foreach ($dateFacets as $dateFacet) {
                $this->responseFacets[] = [
                    'field_name' => $dateFacet,
                    'display_name' => $dateFacet,
                    'counts' => [],
                ];
            }
        }
    ...
    public function getFacetList($filter = null)
    {
        ...
        // Loop through the facets returned by PublicSummon.
        $facetResult = [];
        if (is_array($this->responseFacets)) {
            foreach ($this->responseFacets as $current) {
                // The "display_name" value is actually the name of the field on
                // PublicSummon's side -- we'll probably need to translate this to a
                // different value for actual display!
                $field = $current['display_name'];
        ...
        foreach ($facetResult as $current) {
            $finalResult[$current['display_name']] = $current;
        }
    ...
    protected function formatFacetData($current)
    {
        ...
        foreach ($current['counts'] as $facetIndex => $facetDetails) {
            ...
            // Inject "applied" value into PublicSummon results:
        ...
        // modules to be shared between the Search and PublicSummon modules.
     ...
     *
     * @param array $spelling Suggestions from PublicSummon
     *
     * @return void
     */
    ...
    /**
     * Get best bets from PublicSummon, if any.
     *
    ...
    /**
     * Get database recommendations from PublicSummon, if any.
     *
     * @return array|bool false if no recommendations, detailed array otherwise.
     */
    ...
    /**
     * Get topic recommendations from PublicSummon, if any.
     *
     * @return array|bool false if no recommendations, detailed array otherwise.
     */
    ...
    public function getPartialFieldFacets(
        ...
        // No limit not implemented with PublicSummon: cause page loop
        if ($limit == -1) {
            if ($page === null) {
                $page = 1;
            }
        ...
        if (null !== $facetSort && 'count' !== $facetSort) {
            throw new \Exception("$facetSort facet sort not supported by PublicSummon.");
        }
        ...
        foreach ($facets as $data) {
            if (in_array($data['display_name'], $facetfields)) {
