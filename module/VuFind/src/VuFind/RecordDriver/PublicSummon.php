 ...
 * @link     https://vufind.org/wiki/development:plugins:record_drivers Wiki
 */
class PublicSummon extends DefaultRecord implements Feature\PreviousUniqueIdInterface
{
    ...
    {
        return isset($this->fields['bookmark'])
            ? ['bookmark' => $this->fields['bookmark']] : null;
    }
    ...
    {
        $issns = [];
        if (isset($this->fields['issns'])) {
            $issns = $this->fields['issns'];
        }
        if (isset($this->fields['eissns'])) {
            $issns = array_merge($issns, $this->fields['eissns']);
        }
        return $issns;
    }
    ...
    /**
     * Get the full_title of the record.
     *
     * @return string
     */
    public function getSubtitle()
    {
        if (!isset($this->fields['full_title'])) {
            return '';
        }
        
        $title = $this->fields['full_title'];
        if (is_array($title)) {
            $title = $title[0] ?? '';
        }
